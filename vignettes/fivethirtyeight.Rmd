...
The `r all_fivethirtyeightdata_datasets %>% length()` datasets are included in the `fivethirtyeightdata` add-on package^[The `fivethirtyeightdata` package is hosted via a [`drat` repository](https://cran.r-project.org/web/packages/drat/vignettes/DratForPackageAuthors.html){target="_blank"}]. So for example, to load the `senators` dataset, run:
