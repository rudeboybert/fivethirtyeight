...
vignette: |
  %\VignetteIndexEntry{Data Sets Included, Motivation for Package, and Tame Data Principles}
...
all_fivethirtyeightdata <- datasets %>% 
...
We are aware of [this tweet](https://twitter.com/MonaChalabi/status/1270715883531300866) by Mona Chalabi. Although, we have not decided the future of the `fivethirtyeight` package (and subsequently, the `fivethirtyeightdata` package), we re-iterate that this package is not officially published by 538.

## Note

Some large datasets that could not be hosted in `fivethirtyeight` due to CRAN size restrictions are now included in the `fivethirtyeightdata` add-on package which is hosted via a `drat` repository. Instructions to install `fivethirtyeightdata` can be found in `README.md` of the `fivethirtyeight` package.

There are `r all_datasets %>% length()` datasets between the `fivethirtyeight` and `fivethirtyeightdata` packages. `fivethirtyeight` includes `r nrow(data(package = "fivethirtyeight")[[3]])` datasets while `fivethirtyeightdata` includes the remaining `r all_fivethirtyeightdata %>% length()`.

`fivethirtyeightdata` includes the following datasets:
```{r, message=FALSE, warning=FALSE, echo=FALSE}
all_fivethirtyeightdata
```
