...
Some large datasets that could not be hosted in `fivethirtyeight` due to CRAN size restrictions are now included in the `fivethirtyeightdata` add-on package which is hosted via a `drat` repository. More detail can be found in the "Add-on Package" section of `fivethirtyeight` package `README.md`.
