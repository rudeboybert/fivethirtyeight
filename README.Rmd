...
Some large datasets that could not be hosted in `fivethirtyeight` due to CRAN size resctrictions are included in the `fivethirtyeightdata` add-on package which is hosted by a `drat` repository.
...
Or alternately, you can add the drat repository, so that you can install the `fivethirtyeightdata` package using the `install.packages()` function and update it via the `update.packages()` function:
