# TODO: some variables are not defined in articles or datasets-- is there another source I can check?
#   For "Complete history of the NBA" I wasn't sure about the meaning of the variable "neutral" (consulted outside sources)
#   For "What the world think of trump" I couldn't figure out dk_refused-- maybe percent of respondents who refused to answer?
#   For the "How popular is Donald Trump" first dataset I couldn't figure out "multiversions" variable (maybe if multiple versions of the poll were sent out?) and second dataset I couldn't figure out what the models were based on (it seemed like it should be one model per poll but there were far fewer models than polls)
...
#TODO: Riddler dataset was preloaded in data-raw and is not linked in the article so I'm not sure what to source
...
#'
#'@format A data frame with 3660 rows representing puzzle answers and 2 variables:
#'\describe{
#'  \item{your_number}{Reader answers to the week's Riddler Puzzle}
#'  \item{show_your_work}{Reader explanations for the logic behind their answer}
#'}
#'@source 
...
#'  \item{nych}{The number of emergency hotline (311) calls made to the New York City Housing Authority related to Hurricane Sandy}
...
#'Agency acronyms from the Data Dictionary \url{https://data.cityofnewyork.us/api/views/wewp-mm3p/files/3a563b1d-9e0b-4824-98c7-a308bbc5ce6e?download=true&filename=Call%20Center%20Inquiry%20Data%20Dictionary.xlsx}
#'To convert data frame to tidy data (long) format, run:
#'library(tidyverse)
...
#'  \item{samplesize}{The sample size of the poll}
...
#'  \item{createddate}{Date the poll was created}
...
#'
#'Obama Dataset:
#'@format A data frame with 3207 rows describing individual tweets and 7 variables:
#'\describe{
...
#'}
#'@details Data collected on Oct 20
#'@source Twitter \url(https://twitter.com/BarackObama)
#'
#'Trump Dataset:
#'@format A data frame with 3232 rows describing individual tweets and 7 variables:
#'\describe{
#'  \item{created_at}{The date and time the tweet was posted}
#'  \item{text}{The text of the tweet}
#'  \item{url}{The link to the tweet}
